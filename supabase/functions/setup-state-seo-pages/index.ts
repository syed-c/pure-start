      ...
      try {
        const { error } = await supabaseAdmin
          ...
          .upsert({
            ...
            meta_description: `Discover trusted fostering agencies in ${city.name}, ${state.name}. Compare Ofsted ratings, read carer reviews, and start your fostering journey with Foster Connect.`,
        ...
        try {
          const { error } = await supabaseAdmin
            ...
            .upsert({
              ...
              meta_title: `${treatment.name} in ${city.name} | Foster Connect`,
              meta_description: `Find ${treatment.name.toLowerCase()} services in ${city.name}, ${state.name}. Compare agencies, read reviews, and begin your fostering journey with Foster Connect.`,
              h1: `${treatment.name} in ${city.name}, ${state.name}`,
