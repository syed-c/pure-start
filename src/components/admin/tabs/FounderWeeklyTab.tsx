...
import { 
  TrendingUp, TrendingDown, Calendar, Building2, Users, CheckCircle,
  ArrowUp, ArrowDown, Target, Star, MapPin, AlertTriangle,
  Lightbulb, BarChart3, Banknote
} from 'lucide-react';
import { format, subDays, startOfWeek, endOfWeek } from 'date-fns';

interface WeeklyStats {
  agencies: { thisWeek: number; lastWeek: number; change: number };
  claimed: { thisWeek: number; lastWeek: number; change: number };
  verified: { thisWeek: number; lastWeek: number; change: number };
  enquiries: { thisWeek: number; lastWeek: number; change: number };
...
export default function FounderWeeklyTab() {
  ...
  const { data: weeklyStats, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<WeeklyStats> => {
      ...
      const [
        { count: agenciesThisWeek }, { count: agenciesLastWeek },
        { count: claimedThisWeek }, { count: claimedLastWeek },
        { count: verifiedThisWeek }, { count: verifiedLastWeek },
        { count: enquiriesThisWeek }, { count: enquiriesLastWeek },
      ...
      return {
        agencies: { thisWeek: agenciesThisWeek || 0, lastWeek: agenciesLastWeek || 0, change: calcChange(agenciesThisWeek || 0, agenciesLastWeek || 0) },
        ...
        enquiries: { thisWeek: enquiriesThisWeek || 0, lastWeek: enquiriesLastWeek || 0, change: calcChange(enquiriesThisWeek || 0, enquiriesLastWeek || 0) },
  ...
  const { data: topLocations } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('cities').select('id, name, dentist_count').order('dentist_count', { ascending: false }).limit(5);
      return data || [];
    },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Week of {format(weekStart, 'MMM d')} - {format(weekEnd, 'MMM d, yyyy')}</p>
        </div>
        <Badge variant="outline" className="text-sm py-2 px-4"><Calendar className="h-4 w-4 mr-2" />Week {format(today, 'w')} of {format(today, 'yyyy')}</Badge>
      </div>

      {/* Week-over-Week Comparison Cards */}
      <div className="grid grid-cols-5 gap-4">
        {[
          { label: 'New Agencies', icon: Building2, ...weeklyStats?.agencies, color: 'primary' },
          { label: 'New Claims', icon: CheckCircle, ...weeklyStats?.claimed, color: 'teal' },
          { label: 'Verified', icon: Star, ...weeklyStats?.verified, color: 'gold' },
          { label: 'Enquiries', icon: Calendar, ...weeklyStats?.enquiries, color: 'blue-custom' },
            ...
            <Card key={stat.label} className="card-modern">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className={`h-10 w-10 rounded-xl bg-${stat.color}/10 flex items-center justify-center`}><Icon className={`h-5 w-5 text-${stat.color}`} /></div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="card-modern">
          <CardHeader><CardTitle className="text-lg flex items-center gap-2"><TrendingUp className="h-5 w-5 text-teal" />Agency Retention (30 Days)</CardTitle></CardHeader>
          <CardContent>
            <div className="flex items-center gap-6">
              <div className="text-center">
                <p className="text-4xl font-bold text-teal">{weeklyStats?.retentionRate || 0}%</p>
                <p className="text-sm text-muted-foreground">of claimed agencies active</p>
              </div>
              <div className="flex-1">
                <Progress value={weeklyStats?.retentionRate || 0} className="h-3" />
                <div className="flex justify-between mt-2 text-xs text-muted-foreground">
                  <span>{weeklyStats?.activeAgencies || 0} active</span>
                  <span>of claimed agencies</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="card-modern">
          <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Banknote className="h-5 w-5 text-primary" />Revenue Pipeline</CardTitle></CardHeader>
          <CardContent>
            <div className="flex items-center gap-6">
              <div className="text-center">
                <p className="text-4xl font-bold text-primary">{funnelData?.revenue || 0}</p>
                <p className="text-sm text-muted-foreground">paying agencies</p>
              </div>
              <div className="flex-1 space-y-1 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Conversion Rate</span><span className="font-medium">{funnelData?.claimed ? Math.round((funnelData.revenue / funnelData.claimed) * 100) : 0}%</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Claimed → Verified</span><span className="font-medium">{funnelData?.claimed ? Math.round((funnelData.verified / funnelData.claimed) * 100) : 0}%</span></div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Conversion Funnel */}
      <Card className="card-modern">
        <CardHeader><CardTitle className="text-lg">Marketplace Conversion Funnel</CardTitle><CardDescription>From import to revenue</CardDescription></CardHeader>
        <CardContent>
          <div className="flex items-center justify-between">
            {[
              ...
              <div key={step.label} className="flex items-center flex-1">
                <div className="text-center flex-1">
                  ...
                  {i > 0 && arr[i - 1].value > 0 && (<p className="text-xs text-muted-foreground">{Math.round((step.value / arr[i - 1].value) * 100)}%</p>)}
                </div>
                {i < arr.length - 1 && (<div className="w-12 h-0.5 bg-border" />)}
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card className="card-modern">
          <CardHeader><CardTitle className="text-lg flex items-center gap-2"><MapPin className="h-5 w-5 text-primary" />Top Performing Locations</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {topLocations?.map((loc, i) => (
              <div key={loc.id} className="flex items-center justify-between">
                ...
                <Badge>{loc.dentist_count || 0} agencies</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="card-modern">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2"><Lightbulb className="h-5 w-5 text-gold" />This Week's Priorities</CardTitle>
            <CardDescription>Top 5 actions to improve growth</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {actionTasks?.map((task, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/50">
                <div className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 ${task.priority === 'high' ? 'bg-coral/20' : task.priority === 'medium' ? 'bg-gold/20' : 'bg-muted'}`}>
                  <AlertTriangle className={`h-3 w-3 ${task.priority === 'high' ? 'text-coral' : task.priority === 'medium' ? 'text-gold' : 'text-muted-foreground'}`} />
                </div>
                <div className="flex-1">
      ...
      <Card className="card-modern border-primary/20 bg-gradient-to-r from-primary/5 to-transparent">
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" />AI Platform Health Summary</CardTitle>
          <CardDescription>Automated weekly analysis and recommendations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-background border">
              <div className="flex items-center gap-2 mb-2"><CheckCircle className="h-4 w-4 text-teal" /><span className="font-medium">Strengths</span></div>
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>• Agency onboarding funnel healthy</li>
                <li>• Review collection trending up</li>
                <li>• SEO coverage expanding</li>
              </ul>
            </div>
            <div className="p-4 rounded-xl bg-background border">
              <div className="flex items-center gap-2 mb-2"><AlertTriangle className="h-4 w-4 text-gold" /><span className="font-medium">Watch Areas</span></div>
              <ul className="text-sm text-muted-foreground space-y-1">
            ...
            <div className="p-4 rounded-xl bg-background border">
              <div className="flex items-center gap-2 mb-2"><Target className="h-4 w-4 text-primary" /><span className="font-medium">Priority Actions</span></div>
              <ul className="text-sm text-muted-foreground space-y-1">
          ...
          <p className="text-xs text-muted-foreground mt-4 flex items-center gap-1">
            ...
            AI analysis generated from live platform data.
          </p>
        </CardContent>
      </Card>
    </div>
