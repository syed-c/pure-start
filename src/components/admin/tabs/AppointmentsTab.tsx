...
import {
  Calendar, Search, Clock, CheckCircle, XCircle, AlertTriangle, User, Building2,
  Phone, Heart, Filter, Download, BarChart3, UserCheck, Send, Mail, MessageSquare,
  Link2, Copy, ExternalLink, RefreshCw, Eye, Settings2, Bell, CalendarCheck,
  CalendarX, ArrowUpRight, Zap,
} from 'lucide-react';
...
export default function AppointmentsTab() {
  ...
  const { data: agencyEnquiryCounts } = useDentistBookingCounts();
  ...
  const sendNotification = useMutation({
    ...
    onError: (error) => { toast.error('Failed to send notification'); console.error(error); }
  });
  ...
  const handleStatusChange = async (id: string, status: string, sendEmail: boolean = false) => {
    ...
    if (sendEmail) sendNotification.mutate({ appointmentId: id, type: status });
  };
  ...
  const openManageLink = (token: string) => window.open(`${window.location.origin}/appointment/${token}`, '_blank');
  const openAgencyPage = (slug: string) => window.open(`${window.location.origin}/agency/${slug}`, '_blank');
  ...
  const getFilteredAppointments = () => {
    ...
    switch (activeView) {
      case 'today': filtered = filtered.filter(a => a.preferred_date === today); break;
      case 'upcoming': filtered = filtered.filter(a => a.preferred_date && a.preferred_date >= today && a.status !== 'cancelled' && a.status !== 'completed'); break;
      case 'urgent': filtered = filtered.filter(a => a.status === 'pending' || a.status === 'no_show'); break;
    }
    return filtered;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Enquiry Command Centre</h1>
          <p className="text-muted-foreground mt-1">Manage, track, and optimise all enquiries</p>
        </div>
      ...
      <div className="flex gap-2">
        {[
          ...
          <Button key={tab.key} variant={activeView === tab.key ? 'default' : 'outline'} size="sm" onClick={() => setActiveView(tab.key as any)} className={activeView === tab.key ? '' : 'border-2'}>
            <tab.icon className="h-4 w-4 mr-2" />{tab.label}<Badge variant="secondary" className="ml-2 text-xs">{tab.count}</Badge>
          </Button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <Card key={stat.label} className={`border-2 border-${stat.color}/20 bg-${stat.color}/5 cursor-pointer hover:border-${stat.color}/40 transition-all`}
            ...
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div><p className="text-2xl font-bold text-foreground">{stat.count}</p><p className="text-sm text-muted-foreground">{stat.label}</p></div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      {/* Enquiries by Agency */}
      {agencyEnquiryCounts && agencyEnquiryCounts.length > 0 && (
        <Card className="border border-border">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
              <div className="p-1.5 rounded-lg bg-primary/10"><BarChart3 className="h-5 w-5 text-primary" /></div>
              Enquiries by Agency
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <Select value={filters.clinicId} onValueChange={(v) => setFilters({ ...filters, clinicId: v === 'all' ? '' : v })}>
                  <SelectTrigger className="border-2"><SelectValue placeholder="All Agencies" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All Agencies</SelectItem>
                    {agencies?.map((c) => (<SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                ...
                <Select value={filters.contactId} onValueChange={(v) => setFilters({ ...filters, contactId: v === 'all' ? '' : v })}>
                  ...
                  <SelectContent>
                    ...
                    {contacts?.map((d) => (<SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">Fostering Type</label>
                <Select value={filters.treatmentId} onValueChange={(v) => setFilters({ ...filters, treatmentId: v === 'all' ? '' : v })}>
                  <SelectTrigger className="border-2"><SelectValue placeholder="All Types" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select value={filters.source} onValueChange={(v) => setFilters({ ...filters, source: v === 'all' ? '' : v })}>
                  <SelectTrigger className="border-2"><SelectValue placeholder="All Sources" /></SelectTrigger>
                  <SelectContent>
        ...
        <CardContent className="p-4">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <Input placeholder="Search by name, phone, agency, or contact..." value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} className="pl-10 border-2" />
            </div>
            <Select value={filters.status} onValueChange={(v) => setFilters({ ...filters, status: v === 'all' ? '' : v })}>
              <SelectTrigger className="w-48 border-2"><SelectValue placeholder="All Statuses" /></SelectTrigger>
              <SelectContent>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30">
                ...
                <TableHead>Fostering Type</TableHead>
                    ...
                    <div className="space-y-1">
                      {apt.clinic?.name && (
                        <div className="flex items-center gap-1 text-sm">
                          ...
                          <button onClick={() => apt.clinic?.slug && openAgencyPage(apt.clinic.slug)} className="font-medium hover:text-primary hover:underline cursor-pointer transition-colors">
                            {apt.clinic.name}
                          </button>
                        </div>
                      )}
                      {apt.dentist?.name && (
                        <div className="flex items-center gap-1 text-sm text-muted-foreground"><UserCheck className="h-3 w-3" />{apt.dentist.name}</div>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    {apt.treatment?.name ? (
                      <div className="flex items-center gap-1"><Heart className="h-3 w-3 text-primary" /><span className="text-sm">{apt.treatment.name}</span></div>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {apt.preferred_time && (<p className="text-xs text-muted-foreground">{apt.preferred_time}</p>)}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>{getStatusBadge(apt.status || 'pending')}</TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Select value={apt.status || 'pending'} onValueChange={(v) => handleStatusChange(apt.id, v, true)}>
                        <SelectTrigger className="w-28 h-8 text-xs border-2"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="pending">Pending</SelectItem>
                          <SelectItem value="confirmed">Confirmed</SelectItem>
                          <SelectItem value="completed">Completed</SelectItem>
                        ...
                        <DialogContent className="max-w-lg">
                          ...
                          <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div><p className="text-xs text-muted-foreground">Agency</p><p className="font-medium">{apt.clinic?.name || '-'}</p></div>
                              <div><p className="text-xs text-muted-foreground">Fostering Type</p><p className="font-medium">{apt.treatment?.name || 'General Enquiry'}</p></div>
                            </div>
                            {apt.notes && (<div><p className="text-xs text-muted-foreground mb-1">Notes</p><p className="text-sm bg-muted p-3 rounded-lg">{apt.notes}</p></div>)}
                            {apt.manage_token && (
                              <div className="p-4 bg-primary/5 border border-primary/20 rounded-xl">
                                <p className="text-xs font-medium text-primary mb-2">Self-Service Link</p>
                                <div className="flex gap-2">
                                  <Input value={`${window.location.origin}/appointment/${apt.manage_token}`} readOnly className="text-xs" />
                                  <Button size="sm" variant="outline" onClick={() => copyManageLink(apt.manage_token!)}><Copy className="h-4 w-4" /></Button>
                                  <Button size="sm" onClick={() => openManageLink(apt.manage_token!)}><ExternalLink className="h-4 w-4" /></Button>
                                </div>
                              </div>
                            )}
                            <div className="flex gap-2 pt-2 border-t">
                              ...
                              {apt.clinic?.slug && (
                                <Button variant="outline" size="sm" className="flex-1" onClick={() => openAgencyPage(apt.clinic!.slug!)}>
                                  <Building2 className="h-4 w-4 mr-2" />View Agency
                                </Button>
                              )}
                            </div>
                          </div>
